...
fn main() {
    ...
    total += 1;
    if !test_pattern("cloudflare", full, input) {
        failures += 1;
    }
    ...
    for n in 0..500 {
        ...
        total += 1;
        if !test_pattern(&format!("n={}", n), r"math.*=.*", input_str.as_bytes()) {
            failures += 1;
        }
    }
    ...
    for n in 0..200 {
        ...
        total += 1;
        if !test_pattern(&format!("abcd n={}", n), r"ab.*cd", input_str.as_bytes()) {
            failures += 1;
        }
    }
